...
public static class PolyglotHelper
{
    ...
    {
        return ((int)piece.PieceType - 1) * 2 + (piece.IsWhite ? 1 : 0);
    }

    public static ulong GetPolyglotKey(Board board)
    {
        ulong key = 0;

        // Pieces
        foreach (var pieceList in board.GetAllPieceLists()) foreach (var piece in pieceList)
        {
        ...
        {
            var epSquare = (whiteToMove ? 8*5 : 8*2) + (board.GetEnPassantFile() - 1);
            var epBit = 1UL << epSquare;

            var pawns = board.GetPieceBitboard(PieceType.Pawn, whiteToMove);
